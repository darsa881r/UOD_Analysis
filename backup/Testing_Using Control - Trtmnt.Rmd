...
The basic idea is to award points for correct answers and no rewards for incorrect answers. The percentage of the sum of the points will be the score for the individual student.

Only 5 questions, which varies on the pre-test and post-test, were used for scoring.
...
clean.data$PreScore <- (clean.data$PreScore/5)*100
clean.data$PostScore <- apply(clean.data[,c(9:13)], 1, sum)
clean.data$PostScore <- (clean.data$PostScore/5)*100
...
kable(head(newdata_melted, n=10))
...
It seems that Post-Test scores are higher on average compared to the Pre-Test scores.
...
This bar plot shows comparison between the pre-test scores and post-test scores schools. On average the test scores seems to have improved after the program was offered.
...
Looking at this figure we can certainly say that more percentage of students were able to answer the questions correctly after the program was offered.
...
N = 274 seems to be a large enough sample

Two dependent samples (within-subject design)

Checking the normality:
From the figure below we can say that the distribution is not normal.
...
From the 95% confidence interval it can be said the median difference of the scores are statistically significant at alpha-level 0.95
...
No, we cannot. For that we need to conduct a randomized control trial. From academic literature, it can be said that Quasi-Experimental experiments cannot guarantee causal effect.

This is one of the reasons, we do not conduct control-treatment analysis, as it would not mean anything.

Also, we have to think, are these questions really capturing the essence of behaviour change ??
